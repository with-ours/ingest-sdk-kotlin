...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw OursPrivacyInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw OursPrivacyInvalidDataException("`$name` is not set")
            is JsonNull -> throw OursPrivacyInvalidDataException("`$name` is null")
            else -> throw OursPrivacyInvalidDataException("`$name` is invalid, received $this")
        }
