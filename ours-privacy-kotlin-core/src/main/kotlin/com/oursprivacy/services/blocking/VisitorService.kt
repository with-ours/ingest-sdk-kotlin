...
interface VisitorService {
    ...
    /**
     * Set or update properties on an existing visitor, or create a new visitor if no match is
     * found. This fires a $identify event, making the call visible in the event stream. Identity
     * resolution runs in priority order: userId (direct, no lookup) → externalId (lookup by your
     * ID) → email (fallback lookup). When a visitor is found, their Ours Visitor ID is used going
     * forward so all future events are attached to the same profile. For top-level visitor
